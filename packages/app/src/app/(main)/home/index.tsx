...
import { HomeStore } from "~/modules/home/home.store";
import { trpc } from "~/trpc/provider";

export default function Index() {
  const hydrated = HomeStore.useHydrated();
  const selectedId = HomeStore.useSelectedId();

  const actions = HomeStore.useActions();

  const homes = trpc.home.homes.useQuery(undefined, {
    retry: false,
  });

  if (homes.isLoading || !hydrated) {
    return (
      <View>
        <Text>IsLoading</Text>
      </View>
  ...
  if (selectedId) {
    return <Redirect href={`/${selectedId}`} />;
  }
      ...
      <View>
        {homes.data.map((home) => (
          <Pressable
            key={home.id}
            onPress={() => {
              actions.chooseHome(home.id);
              router.push(`/${home.id}`);
            }}
            className="p-2 border border-neutral-800 bg-neutral-900 rounded-md"
          >
            <View className="flex-row gap-x-2 items-center">
              <View style={{ backgroundColor: home.colorHex }} className="h-4 w-4 rounded-md" />
...
function HomeName(props: HomeNameProps) {
  ...
  return (
    <View className="gap-y-4">
      <Text className="text-neutral-200">Name of your home</Text>
...
function HomeColor(props: HomeColorProps) {
  ...
  return (
    <View className="gap-y-4">
      <Text className="text-neutral-200">Choose a color for your home</Text>
  ...
  return (
    <View className="gap-4">
      <View className="gap-x-4 flex-row">
        <View style={{ backgroundColor: props.color }} className="w-8 h-8 rounded-md" />
        <Text className="text-2xl text-neutral-200 font-bold">{props.name}</Text>
      </View>
      <Button
        onPress={() => {
      ...
      >
        <ButtonText>Create home</ButtonText>
      </Button>
    </View>
  );
}
